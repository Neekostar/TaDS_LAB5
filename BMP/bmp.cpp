...
BMP::BMP(int32_t width, int32_t height, bool has_alpha) {
    ...
    if (width <= 0 || height <= 0) throw MyGraph::Errors{"The image width and height must be positive numbers."};
...
void BMP::load(const std::string & file_path) {
    ...
    if (!input) throw MyGraph::Errors{"Unable to open the input image file!"};

    // считываем заголовочную структуру изображения
    input.read((char*)&m_file_header, sizeof(m_file_header));

    // проверяем формат изображения
    if (m_file_header.file_type != 0x4D42) throw MyGraph::Errors{"Unrecognized file format!"};
    ...
    if (m_info_header.bit_count == 32) {
        ...
        if (m_info_header.size >= (sizeof(InfoHeader) + sizeof(ColorHeader))) {
            ...
            throw MyGraph::Errors{"Error! Unrecognized file format!"};
        }
    }
    ...
    if (m_info_header.height < 0)
        throw MyGraph::Errors{"The program can treat only BMP images with the origin in the bottom left corner!"};
...
void BMP::save(const std::string & file_path) {
    ...
    // проверяем, открылся ли файл
    if (!output) throw MyGraph::Errors{"Unable to open the input image file!"};
    ...
    if (m_info_header.bit_count == 32) {
        ...
        throw MyGraph::Errors{"The program can treat only 24 or 32 bits per pixel BMP files"};
    }
}
...
void BMP::fillRegion(uint32_t x0, uint32_t y0, uint32_t width, uint32_t height,
    ...
    if (x0 + width > (uint32_t)m_info_header.width || y0 + height > (uint32_t)m_info_header.height)
        throw MyGraph::Errors{"The region does not fit in the image!"};
...
void BMP::_checkColorHeader(ColorHeader &color_header) {
    ...
    if (expected_color_header.red_mask   != color_header.red_mask   ||
        ...
        throw MyGraph::Errors{"Unexpected color mask format! "
    ...
    if (expected_color_header.color_space_type != color_header.color_space_type) {
        throw MyGraph::Errors{"Unexpected color space type! The program expects sRGB values"};
    }
}
...
void BMP::createRectangle(){
    ...
    // найдем диаметр будущей окружности (минимум между шириной и высотой)
    // координаты центра окружности
    int32_t x0 = width/2;
    int32_t y0 = height/2;

    fillRegion(x0, y0, 100, 80, red, green, blue, alpha);

}

void BMP::resizeImage() {
    uint32_t channels = m_info_header.bit_count / 8;
    // получаем реальные размеры изображения
    int32_t width = getWidth();
    int32_t height = getHeight();
    // найдем диаметр будущей окружности (минимум между шириной и высотой)
    int32_t diametr = std::min(width, height);
    int32_t radius = diametr/2;
    // координаты start
    int32_t x0 = (width - radius*2)/2;
    int32_t y0 = 0;
    //new size
    int32_t new_height = diametr-1;
    int32_t new_width = diametr-1;
    // выделяем память для информации о пикселях
    std::vector<uint8_t> temp(new_width * new_height * channels);
    ...
    for (uint32_t y = y0; y<new_height; ++y ){
        for (uint32_t x = x0; x<x0+diametr; ++x) {
            temp[channels * (f_y * new_width + f_x) + 0] = m_data.at(channels * (y * m_info_header.width + x) + 0);
            temp[channels * (f_y * new_width + f_x) + 1] = m_data.at(channels * (y * m_info_header.width + x) + 1);
            temp[channels * (f_y * new_width + f_x) + 2] =  m_data.at(channels * (y * m_info_header.width + x) + 2);
            if (channels == 4) {
                temp[channels * (f_y * new_width + f_x) + 3] =  m_data.at(channels * (y * m_info_header.width + x) + 3);
            }
            f_x++;
        }
        f_y++;
